...
class TestImbalancedClass(unittest.TestCase):
    filename = os.getcwd() + "\\dataset\\wrangling_train_dataframe.csv"
    target = 'ProdTaken'

    def _get_target_distribution(self, dataframe: pd.DataFrame):
        counts = dataframe[TestImbalancedClass.target].value_counts().to_list()
    ...
    def test_RandomUnderSampler(self):
        ...
        actual_df = pd.read_csv(TestImbalancedClass.filename)
        ...
        random_under_sampler = ImbalancedClass(dataframe=actual_df,
                                               target=TestImbalancedClass.target,
                                               type='UnderSampling',
                                               method='Random',
        ...
        print(
            f"Distribution of target in expected df: {expected_df_majority_class_size, expected_df_minority_class_size}")
